document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchRequests() {
        try {
            const response = await fetch(`http://localhost:3000/api/requests/${user.id}`);
    ...
    function renderRequests(requests) {
        if (requests.length === 0) {
            container.innerHTML = '<p style="text-align: center; color: #666; padding: 20px;">You have no incoming requests at the moment.</p>';
            return;
        }
