document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const response = await fetch(`/api/rentals/renter/${user.id}`);
  ...
  function renderBookings(bookings) {
    if (bookings.length === 0) {
      rentalsContainer.innerHTML =
        '<p style="text-align:center; padding: 20px;">You have no active bookings yet.</p>';
      return;
    }
    ...
    bookings.forEach((booking) => {
      ...
      const badgeClass =
        booking.Status === "Pending" ? "badge-pending" : "badge-approved";
      ...
      const itemHTML = `
                <div class="rental-item">
                    <img src="${imagePath}" alt="${booking.Category}" class="rental-thumb" style="width:80px; height:80px; object-fit:cover; border-radius:4px;">
